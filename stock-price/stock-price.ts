import { Rectangle, type Point, SetSVGContext, DrawGrid, DrawAxes, MakePolyLine, type SvgContext, MakeSVGContext, SetBackgroundGradient, 
    SetPoints} from "../modules/svg-draw.js"
import {assert, log} from "../modules/misc-tools.js"
import { AddDataValue, TotalValuesInBins, MaxValueInBins, MakeHistogramView, GetDimensions, RefreshSVG, type HistogramView} from "../modules/histogram.js"
...
let svgElement = document.querySelector("#svgRoot") as SVGSVGElement
let ctx = MakeSVGContext(svgElement, USER_X_MAX, USER_Y_MAX)
SetSVGContext(ctx)

DrawGrid(USER_X_MAX, USER_Y_MAX)
DrawAxes()

let curPoints : Point[] = [{x: -USER_X_MAX, y: 0}]
let curPolyLine = MakePolyLine(curPoints)
let startNewPathOnNextTick = true
let histogramView = InitializeHistogram(ctx);
...
log("I'm running the latest, greatest, typescript version!")
...
function InitializeHistogram(ctx: SvgContext) : HistogramView
{
    let bins = [];
    ...
    let histogramView = MakeHistogramView(bins, {}, ctx);
