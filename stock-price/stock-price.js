import { Rectangle, Line, SetSVGContext, DrawGrid, DrawAxes, MakePolyLine, MakeSVGContext, SetBackgroundGradient, SetPoints } from "../modules/svg-draw.js";
import { assert, log } from "../modules/misc-tools.js";
import { AddDataValue, MakeHistogramView, RefreshSVG } from "../modules/histogram.js";
...
let brownianSvg = document.querySelector("#svg1");
let brownianCtx = MakeSVGContext(brownianSvg, BROWNIAN_BOUNDS);
let stockSvg = document.querySelector("#svg2");
...
DrawGrid(stockCtx);
DrawAxes(stockCtx);
let brownianCurPoints = [{ x: 0, y: 0 }];
let stockCurPoints = [{ x: 0, y: S0 }];
...
window.state = state;
SetUpKeyListeners();
const SIM_TICKS_PER_SECOND = 10000; // (Could go up to 10 million at 60fps!!! In that case, for large numbers, have to add nudges to prevent weird behaviour, I think having to do with bigints...)
...
function InitializeBrownianHistogram(ctx) {
    assert(ctx);
    let bins = [];
    ...
    let histogram = { bins, totalValuesInBins };
...
function InitializeStockHistogram(ctx) {
    assert(ctx);
    let bins = [];
...
function GetRandomBrownianJump() {
